...
const payload = { repository: { id: 1350979676, full_name: '4444J99/organvm-ci-relay' }, workflow_run: { id: 7, run_attempt: 1, path: '.github/workflows/relay-policy.yml', event: 'pull_request_target', status: 'completed', conclusion: 'success', head_sha: sha, pull_requests: [{ number: 29, head: { sha }, base: { ref: 'main', sha: base, repo: { id: 1350979676 } } }] } };
...
for (const [name, mutate] of [
  ['run SHA differs from candidate', p => { p.workflow_run.head_sha = base; }],
...
test('unrelated PR does not mask the single matching main PR', () => {
  const p = structuredClone(payload); p.workflow_run.pull_requests.push({ number: 90, head: { sha }, base: { ref: 'release', repo: { id: 1350979676 } } });
  assert.equal(evaluateWorkflowRun(p, payload.repository.full_name, 1350979676).admitted, true);
});
test('repository ID must be configured, never inferred from webhook', () => {
  assert.equal(evaluateWorkflowRun(payload, payload.repository.full_name, null).eligible, undefined);
});
test('trusted failures remain eligible for canonical failure publication', () => {
  const p = structuredClone(payload); p.workflow_run.conclusion = 'failure';
  const out = evaluateWorkflowRun(p, payload.repository.full_name, 1350979676);
  assert.equal(out.eligible, true); assert.equal(out.admitted, false);
});
